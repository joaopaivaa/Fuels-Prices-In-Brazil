...
df = (
    df
    .withColumn("nm_city_unaccented", remove_accents_udf("nm_city"))
    .withColumn("nm_state_unaccented", remove_accents_udf("nm_state"))
)

df = (
    df
    .withColumn('key_uf_city_lower', concat(lower(col('ab_state')), lit('_'), regexp_replace(lower(col('nm_city_unaccented')), ' ', '_')))
)
